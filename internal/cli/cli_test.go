...
func (f *fakeService) GetBuildLog(ctx context.Context, buildID string) (string, error) {
	return f.log, nil
}
func (f *fakeService) RerunBuild(ctx context.Context, buildID string) (string, error) {
...
func TestListProjects(t *testing.T) {
	...
	code := c.Execute(context.Background(), []string{"list"})
	if code != 0 {
...
func TestListProjectsAliasLS(t *testing.T) {
	...
	var out, err bytes.Buffer
	c := &CLI{Service: svc, Out: &out, Err: &err}
	code := c.Execute(context.Background(), []string{"ls"})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; err=%s", code, err.String())
	}
	got := out.String()
...
func TestListProjectBuilds(t *testing.T) {
	...
	var out, err bytes.Buffer
	c := &CLI{Service: svc, Out: &out, Err: &err}
	code := c.Execute(context.Background(), []string{"list", "proj"})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; err=%s", code, err.String())
	}
	got := out.String()
...
func TestLessUsage(t *testing.T) {
	...
	var out, err bytes.Buffer
	c := &CLI{Service: svc, Out: &out, Err: &err}
	code := c.Execute(context.Background(), []string{"less"})
	if code == 0 {
...
func TestLessAndRerun(t *testing.T) {
	...
	code := c.Execute(context.Background(), []string{"less", "b1"})
	if code != 0 {
	...
	code = c.Execute(context.Background(), []string{"rerun", "b1"})
	if code != 0 {
